...
\begin{proof}
  ...
  \item ($1 \implies 2$)
    ...
    Let $U_{\rho}$ be the variables corresponding to the bits of the proof string $\pi$ queried by $V$ on input $\rho$.

    Now we construct a reduction from $L$ to \textsc{$\frac{1}{2}$-gap $q$-CSP}.
    Define the reduction $f$ by $f(x) = (\Gamma, U, D)$, where
    \begin{align*}
      \Gamma & = \{0, 1\}, \\
      U & = \bigcup_{\rho \in \Sigma^{r \lg n}}{U_\rho}, \text{ and} \\
  ...
  \item ($2 \implies 1$)
    ...
    Finally, it checks that the constraint is sastisfied, by querying the proof string at the appropriate locations (the locations corresponding to the $q$ variables in the constraint).
