...
As an analog to \autoref{prop:equivalence}, we can provide an inapproximability view of the restricted version of the PCP Theorem.
The statement of this proposition is more precise than that of \autoref{prop:equivalence} about the number of bits of randomness required because the precision is required for a later theorem.

\begin{proposition}\label{prop:inapprox}
  Suppose $d$ is a non-negative integer and $k$ is an integer greater than or equal to 1 (\textbf{TODO is $k = 1$ okay?}).
  The following are equivalent.
  \begin{enumerate}
    \item There is a non-negative integer $t$ such that
      \begin{equation*}
        \NNC^k(\lg^d n) \subseteq \PCPcs{\NC^k}{1}{\frac{1}{2}}{d \lg \lg n + t}{O(1)}.
      \end{equation*}
    \item There exists an integer $q$ with $q > 1$ such that \textsc{$\frac{1}{2}$-gap $q$-CSP} is hard for $\NNC^k(\lg^d n)$ under $\leq_m^{NC^k}$ reductions that produce instances with $O(\lg^d n)$ variables and $O(\lg^d n)$ constraints.
  \end{enumerate}
\end{proposition}
\begin{proof}
...
The results in this section are \NC{} analogs of results from \cite{fs96}.
...
%% TODO can we replace this with a generic pseudo random generator?
...
Using the derandomizations from \autoref{prop:pcpinnnc} and \autoref{lem:derandomize}, we can prove the following theorem.
...
\begin{theorem}
  ...
  \begin{enumerate}
  \item $\NNC^k(\lg^d n) = \PCPcs{\NC^k}{1}{\frac{1}{2}}{d \lg \lg n}{q}$.
...
\begin{proof}
  ...
  By hypothesis, $L \in \PCPcs{\NC^k}{1}{\frac{1}{2}}{d \lg \lg n}{q}$.
...
The technique used in the proof of the preceding theorem also yields the following corollary.
...
Combining these results with \autoref{prop:inapprox} yields evidence that the techniques used to prove the PCP Theorem do not provide the insight necessary to derive a PCP characterization for $\NNC(\polylog)$.

\begin{corollary}
  Let $k$ be a positive integer and $d$ be a non-negative integer.
  If there exists an integer $q$ with $q > 1$ such that \textsc{$\frac{1}{2}$-gap $q$-CSP} is hard for $\NNC^k(\lg^d n)$ under $\leq_m^{NC^k}$ reductions that produce instances with $O(\lg^d n)$ variables and $O(\lg^d n)$ constraints, then $\NNC^k(\lg^d n) = \NC^k$.
\end{corollary}

\begin{corollary}
  If there exists an integer $q$ with $q > 1$ such that \textsc{$\frac{1}{2}$-gap $q$-CSP} is hard for $\NNC(\polylog)$ under $\leq_m^{NC}$ reductions that produce instances with $O(\polylog)$ variables and $O(\polylog)$ constraints, then $\NNC(\polylog) = \NC$.
\end{corollary}
