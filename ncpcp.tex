...
\begin{proposition}\label{prop:pcpinnnc}
  For all $j \in \mathbb{N}$ with $j \geq 1$, for all $k \in \mathbb{N}$ with $k \geq 1$ (\textbf{TODO can this be k = 1?}), and for all positive rationals $q$ and $r$,
  \begin{equation*}
    \PCPcs{\NC^k}{1}{\frac{1}{2}}{r \lg n}{2^{q \lg^j n}} \subseteq \NNC^k(\lg^j n).
  \end{equation*}
\end{proposition}
\begin{proof}
  Let $L \in \PCPcs{\NC^k}{1}{\frac{1}{2}}{r \lg n}{2^{q \lg^j n}}$.
  Let $V$ be the \PCP{} verifier for $L$ which, on input $x$ of length $n$, proof string $\pi$, and random string $\rho$, makes exactly $2^{q \lg^j n}$ (nonadaptive) queries to the proof string and uses exactly $r \lg n$ random bits, and which satisfies
  \begin{enumerate}
  \item if $x \in L$, then
    \begin{equation*}
      \exists \pi \in \Sigma^{r \lg n + \lg{2^{q \lg^j n}}} \colon \Pr_{\rho \in \Sigma^{r \lg n}}{\left[V(x, \pi; \rho) \textnormal{ accepts}\right]} = 1,
    \end{equation*}
    and
  \item if $x \notin L$, then
    \begin{equation*}
      \forall \pi \in \Sigma^{r \lg n + \lg{2^{q \lg^j n}}} \colon \Pr_{\rho \in \Sigma^{r \lg n}}{\left[V(x, \pi; \rho) \textnormal{ accepts}\right]} < \frac{1}{2}.
    \end{equation*}
  \end{enumerate}
  Our goal is to construct a $\NNC^k(\lg^j n)$ machine which accepts $x$ if and only if $x \in L$ by simulating the \PCP{} verifier $V$.

  Define the $\NNC^k(\lg^j n)$ machine $M$ as follows on input $x$ of length $n$:
  \begin{enumerate}
  \item Guess a proof string $\pi$ of length $r \lg n + \lg{2^{q \lg^j n}}$.
  \item For each $\rho$ of length $r \lg n$ \emph{in parallel}, simulate $V(x, \pi; \rho)$ and let $b_\rho$ be its output bit.
  \item Let
    \begin{equation*}
      B = \sum_{\rho \in \Sigma^{r \lg n}}{b_\rho},
    \end{equation*}
    and let
    \begin{equation*}
      B_0 = \frac{1}{2} 2^{r \lg n}.
    \end{equation*}
  \item Accept if and only if $B \geq B_0$.
  \end{enumerate}

  First we show that $M$ can be implemented on a polynomial number of processors with parallel running time $\lg^k n$ and with at most $O(\lg^j n)$ nondeterministic gates.
  $M$ uses exactly $r \lg n + \lg{2^{q \lg^j n}}$ nondeterministic gates (all in the first step), or more simply, $r \lg n + q \lg^j n$, which is in $O(\lg^j n)$ for all $j \geq 1$.
  In step 2, $M$ runs $2^{r \lg n}$, or more simply $n^r$ (a polynomial in $n$), instances of $V$ in parallel; the parallel running time of $V$ itself is $O(\lg^k)$, so the parallel running time of this step in $M$ is the same.
  In step 3, $M$ computes the sum of $2^{r \lg n}$ numbers, or more simply $n^r$ numbers (each a single bit, so the total size of $B$ is $\lg n^r$, which equals $r \lg n$), and computes the value of $\frac{1}{2} 2^{r \lg n}$, or $\frac{1}{2} n^r$.
  The sum of $n^r$ numbers can be computed by an \NC{} circuit of size $O(n^r)$ and depth $O(\lg n)$ \cite[Theorem~2.6.1]{savage98}.

  Now we show the correctness of $M$.
  If $x \in L$, then all instances of $V$ accept, so $B = 1 \geq \frac{1}{2} n^r = B_0$, and hence $M$ accepts.
  If $x \notin L$, then less than half of the instances of $V$ accept, so $B < \frac{1}{2} n^r = B_0$, and hence $M$ rejects.
  Therefore we have shown that $M$ is a correct algorithm for $L$ which uses a polynomial number of processors, has $O(\lg^k n)$ running time, and uses $O(\lg^j n)$ nondeterministic gates.
...
\begin{corollary}
  ...
  \begin{enumerate}
  \item $\PCPcs{\NC^k}{1}{\frac{1}{2}}{0}{0} = \PCPcs{\NC^k}{1}{\frac{1}{2}}{O(\lg n)}{O(\poly)} = \NNC^k(\lg n) = \NC^k$,
  \item $\PCPcs{\NC^k}{1}{\frac{1}{2}}{O(\lg n)}{2^{O(\polylog)}} \subseteq \NNC^k(\polylog)$.
  \end{enumerate}
\end{corollary}
...
\begin{corollary}
  \mbox{}
  \begin{enumerate}
  \item $\PCPcs{\NC}{1}{\frac{1}{2}}{0}{0} = \PCPcs{\NC}{1}{\frac{1}{2}}{O(\lg n)}{O(\poly)} = \NC$,
  \item $\PCPcs{\NC}{1}{\frac{1}{2}}{O(\lg n)}{2^{O(\polylog)}} \subseteq \NNC(\polylog)$.
  \end{enumerate}
\end{corollary}
