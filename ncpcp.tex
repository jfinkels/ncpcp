...
\begin{proposition}
  If a language $L$ is complete for $\NNC^k(\lg^d n)$ under $\leq_m^{NC^k}$ reductions and $L \in \NC^k$ then $\NNC^k(\lg^d n) = \NC^k$.
\end{proposition}
...
\begin{lemma}[{{\cite[Lemma~3.2.8]{wolf90}}}]
...
As an analog to \autoref{prop:equivalence}, we can again provide an inapproximability view of the restricted version PCP Theorem.
...
\begin{proof}
  ...
  The reduction in the proof that 1 implies 2 now enumerates (in parallel) each of the $2^{d \lg \lg n}$ possible random strings of length $d \lg \lg n$ instead of $2^{r \lg n}$ possible strings of length $r \lg n$.
  Similarly, the PCP verifier in the proof that 2 implies 1 requires exactly $d \lg \lg n$ bits of randomness to select one of the $\lg^d n$ constraints uniformly at random.
\end{proof}
...
\begin{definition}
  Let $P$ and $Q$ be maximization problems with $P = (I_P, S_P, m_p, \max)$ and $Q = (I_Q, S_Q, m_Q, \max)$.
  ...
  \begin{enumerate}
  \item if $m^*_P(x) \geq c_P(x)$ then $m^*_Q(f(x)) \leq c_Q(f(x))$,
