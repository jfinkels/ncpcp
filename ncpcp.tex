...
%% TODO this is true only if it turns out that q(n) and r(n) have the correct
%% values in \cite{bcgt12}
We also know the following strengthening of the original PCP Theorem. \textbf{TODO don't know if thats what BCGT12 is showing...}

\begin{theorem}[{\cite[Section~2.1]{bcgt12}}]\label{thm:pcpnp}
  $\PCP^\NC[O(\log n), O(1)] = \NP$.
\end{theorem}

In \cite{bcgt12}, it seems that the authors actually show an \NC{} PCP verifier for the universal complete problem $U$ (also called the bounded halting problem) for random access machines (RAMs).
For Turing machines, the problem is \NP-complete by a simple generic reduction.
The problem remains \NP-hard for RAMs by reducing a Turing machine to a RAM in polynomial time by a straightforward simulation.
Hence an \NC{} PCP verifier for the universal complete problem for RAMs implies the necessary inclusion of complexity classes.
For an alternate proof of \autoref{thm:pcpnp}, see \autoref{sec:alternate}.
...
This makes sense: the ``difference'' between $\log n$ and a polynomial in $\log n$ is less than the ``difference'' between a polynomial in $\log n$ and a polynomial in $n$.
...
(However, it could still be the case that for some fixed positive integer $k$, we have $\PCP^\NC[O(\loglog n), O(1)] = \PCP^\NC[O(\loglog n), O(\log^k n)]$; we \textbf{CONJECTURE} that this is true for $k = 1$.)
...
\end{equation*}
which provides an even more severe collapse.
\begin{theorem}
  $\PCP^\NC[\poly(\loglog n), O(\log n)] = \NC$.
\end{theorem}
\begin{proof}
  Using the fact that $O(\log^\alpha \log n) \subseteq O(\log n)$ for all non-negative integers $\alpha$ and the \textbf{CONJECTURE} that
  \begin{equation*}
    \PCP^\NC[O(\loglog n), O(\log n)] \subseteq \PCP^\NC[O(\loglog n), O(1)],
  \end{equation*}
  we have
  \begin{align*}
...
\section{Alternate proof of highly parallel probabilistically checkable proofs for \texorpdfstring{\NP}{NP}}\label{sec:alternate}
