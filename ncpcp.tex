...
\begin{proof}
  ...
  For the converse, suppose $L$ is a language in $\NNC^k(\log^d n)$.
  By hypothesis, $L \in \PCP^{\NC^k}[d \loglog n + t, q]$ for some non-negative integer $t$.
  Since $d \loglog n + t$ is in $O(\loglog n)$, we also have that $L \in \PCP^{\NC^k}[d' \loglog n, q]$ for some $d'$ greater than or equal to $d$.
  By \autoref{lem:derandomize}, there exists a $t'$ such that $L \in \PCP^{\NC^k}[\loglog n + t', q]$.
