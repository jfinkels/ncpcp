...
We first provide a PCP characterization of $\NNC(\polylog)$, then later we provide upper and lower bounds for the parameters of such a PCP verifier.
...
\begin{proof}
  ...
  \end{enumerate}
In the initial step, guessing a proof string requires $O(\log^{r + q} n)$ bits.
...
\end{lemma}
\begin{proof}
  \P{} has complete languages under logarithmic space many-one reductions; \textsc{Boolean Formula Evaluation}, for example.
  ...
  The composition of the reduction with the $O(\log^k n)$ space algorithm that decides $L$ yields a $O(\log^k n)$ space algorithm for $B$.
...
\begin{theorem}\label{thm:pinpcp}
  If $\P \subseteq \PCP^\NC[O(\loglog n), \polylog]$ then $\P \subsetneq \polyL$.
\end{theorem}
\begin{proof}
  By \autoref{thm:qplusr}, we have $\P \subseteq \NNC(\polylog)$.
  By \cite[Corollary~3.2]{wolf94}, we have $\P \subseteq \polyL$.
  By \autoref{lem:pneqpolyl}, we have $\P \subsetneq \polyL$.
\end{proof}

\begin{todo}
  Provide evidence that $\P \subsetneq \polyL$ is unlikely.
\end{todo}
...
\subsection{Approximability of constraint satisfaction problems}

In \cite{ep06} and \cite{kmsv99} the authors discuss optimization problems which are ``canonically hard'' for classes of approximable optimization problems.
Roughly, an optimization problem is canonically hard if there is a certain kind of reduction from all \NP{} decision problems in which an \NP{} witness can be reconstructed for the decision problem given a good enough approximate solution to the optimization problem.
In \cite[Theorem~4]{kmsv99}, the authors state that the original PCP theorem from \cite{almss92} implies that \textsc{Maximum 3-Satisfiability} is canonically hard for \APX, the class of \NP{} optimization problems for which there is a constant factor approximation algorithm.
Does Dinur's proof imply that \textsc{Maximum $q$-CSP}, the optimization problem corresponding to \textsc{$q$-CSP}, is canonically hard for \APX?
In other words, can we reconstruct a satisfying assignment to the original constraint graph given an assignment satisfying at least half of the constraints of the constraint graph that results from Dinur's process described in the previous section?

In \cite[Corollary~13]{trevisan98}, the author states that \textsc{Maximum $q$-CSP} has an \NC{} approximation scheme (which implies a polynomial time approximation scheme).
If this problem were canonically hard for \APX, this would imply $\APX = \PTAS$, which is true if and only if $\P = \NP$.
Therefore we conclude the following.
\begin{theorem}
  \textsc{Maximum $q$-CSP} is canonically hard for \APX if and only if $\P = \NP$.
\end{theorem}
What does this say about Dinur's reduction?
It means that a satisfying assignment for the original constraint graph cannot be reconstructed from an assignment to the transformed constraint graph that satisfies most of the constraints.
\begin{todo}
  Is this correct?
\end{todo}
