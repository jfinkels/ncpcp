...
\begin{proof}
  ...
  \begin{equation*}
    \NNC(\polylog) \subseteq \PCP^{\NC}[0, \polylog] \subseteq \PCP^{\NC}[O(\loglog n), \polylog].
  \end{equation*}
  For the other direction, suppose $L \in \PCP^{\NC}[r \loglog n, c \log^q n]$ for some constants $r$, $q$, and $c$.
...
(However, it could still be the case that for some fixed $k$, we have $\PCP^\NC[O(\loglog n), O(1)] = \PCP^\NC[O(\loglog n), O(\log^k n)]$.)
...
\textbf{TODO add information about PCP(poly loglog n, ...)}
...
\textbf{TODO fill me in with possible consequences of PCP characterization for P}
