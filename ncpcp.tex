...
\begin{lemma}[{\cite[Lemma~3.1 and Lemma~3.3]{wolf94}}]\label{lem:dspace}
  ...
  \begin{enumerate}
  \item $\NNC^k(\log^j n) \subseteq \DSPACE(\log^{\max(k, j)} n)$ and
  \item $\NNC^k(n^j) \subseteq \DSPACE(n^j)$.
  \end{enumerate}
\end{lemma}

$\NNC^k(\log^d n)$ has complete problems under logarithmic space many-one reductions \cite[Theorem~3.6]{cc97}, so that part of the hypothesis in item 2 of the next proposition makes sense.

\begin{proposition}
  For all integers $k$ and $d$,
  \begin{enumerate}
  \item if a language $L$ is hard for $\NNC^k(\log^d n)$ under $\leq_m^P$ reductions and $L \in \P$ then $\NNC^k(\log^d n) \subseteq \P$, and
  \item if a language $L$ is hard for $\P$ under $\leq_m^{NC^k}$ reductions and $L \in \NNC^k(\log^d n)$ then $\P \subseteq \NNC^k(\log^d n)$.
    Furthermore $\P \subsetneq \polyL$.
  \end{enumerate}
\end{proposition}
\begin{proof}
  A standard simulation argument proves both statements.
  Since we have $\NNC^k(\log^d n) \subseteq \DSPACE(\log^m)$, where $m = \max(k, d)$, by \autoref{lem:dspace} and $\P \neq \polyL$ (by the Space Hierarchy Theorem), we conclude that $\P \subsetneq \polyL$.
\end{proof}
...
\begin{lemma}\label{lem:pcpinnp}
  $\PCP^\P[O(\log n), O(1)] \subseteq \NP$.
\end{lemma}
\begin{proof}
  Let $L \in \PCP^\P[O(\log n), O(1)]$, so there exists a polynomial time PCP verifier $V$ subject to the appropriate restrictions.
  Construct a nondeterministic Turing machine $N$ as follows.

  On input $x$, nondeterministically guess a proof string $\pi$ of length $2^{r \log n + \log q}$, where $q$ and $r$ are the appropriate constants for the language $L$.
  Enumerate each ``random'' string $\rho$ of length $r \log  n$, and for each $\rho$, simulate $V$ on input $x$, proof string $\pi$, and random string $\rho$.
...
\begin{proposition}[{{\cite[Theorem~1.3]{dinur07}}}]\label{prop:equivalence}
  ...
  \begin{enumerate}
  \item $\NP \subseteq \PCP^\P[O(\log n), O(1)]$.
...
\begin{lemma}\label{lem:constraintsp}
  If $L \in \PCP^\P[O(\log n), O(1)]$ then there is a $\leq_m^P$ reduction from $L$ to \textsc{$\frac{1}{2}$-gap $q$-CSP}, for some integer $q$ greater than 1.
\end{lemma}
\begin{proof}
  Let $L$ be a language in $\PCP^\P[O(\log n), O(1)]$.
  Let $V$ be the polynomial time PCP verifier for $L$ which uses $r \log n$ bits of randomness and queries $q$ bits of the proof string, for some constants $r$ and $q$.
  ...
  For each fixed input $x$ of length $n$ and each fixed $\rho \in \Sigma^{r \log n}$, simplify the circuit based on the values (that is, the 0s and 1s) of the variables corresponding to $x$ and the variables corresponding to $\rho$ into a Boolean function (also called a Boolean constraint) which depends only on the bits of the proof string $\pi$.
  ...
  \begin{align*}
    \Gamma & = \{0, 1\}, \\
    U & = \bigcup_{\rho \in \Sigma^{r \log n}}{U_{x, \rho}}, \text{ and} \\
    D & = \left\{\phi_{x, \rho}\right\}_{\rho \in \Sigma^{r \log n}}.
  \end{align*}
...
\end{proof}
With this equivalence, we can reduce the problem of showing that $\NP \subseteq \PCP^\P[O(\log n), O(1)]$ to the problem of showing that \textsc{$\frac{1}{2}$-gap $q$-CSP} is \NP-hard.
...
\begin{proposition}\label{prop:pcpinnnc}
  ...
  \begin{equation*}
    \PCP^{\NC^k}[r \loglog n + t, q \log^j n] \subseteq \NNC^k(\log^{r + j} n).
  \end{equation*}
\end{proposition}
\begin{proof}
  Let $L \in \PCP^{\NC^k}[r \loglog n + t, q \log^j n]$.
  Let $V$ be the PCP verifier for $L$ which, on input $x$ of length $n$, proof string $\pi$, and random string $\rho$, makes at most $q \log^j n$ (nonadaptive) queries to the proof string and uses at most $r \loglog n + t$ random bits, and which satisfies
  \begin{enumerate}
  \item if $x \in L$, then there exists a $\pi$ of length $2^{r \loglog n + t} \cdot q \log^j n$ such that
    \begin{equation*}
      \Pr_{\rho \in \Sigma^{r \loglog n + t}}{\left[V(x, \pi; \rho) \textnormal{ accepts}\right]} = 1,
    \end{equation*}
    and
  \item if $x \notin L$, then for all $\pi$ of length $2^{r \loglog n + t} \cdot q \log^j n$,
    \begin{equation*}
      \Pr_{\rho \in \Sigma^{r \loglog n + t}}{\left[V(x, \pi; \rho) \textnormal{ accepts}\right]} < \frac{1}{2}.
    \end{equation*}
  \end{enumerate}
  Our goal is to construct a $\NNC^k(\log^{r + j} n)$ machine which accepts $x$ if and only if $x \in L$ by simulating the PCP verifier $V$.

  Define the $\NNC^k(\log^{r + j} n)$ machine $M$ as follows on input $x$ of length $n$:
  \begin{enumerate}
  \item Guess a proof string $\pi$ of length $2^{r \loglog n + t} \cdot q \log^j n$.
  \item For each $\rho$ of length $r \loglog n + t$ \emph{in parallel}, simulate $V(x, \pi; \rho)$ and let $b_\rho$ be its output bit (that is, 1 if $V$ accepts and 0 if $V$ rejects).
  \item Let $B$ equal the sum of the output bits for each of the above simulations, and let $B_0 = \frac{1}{2} 2^{r \loglog n + t}$.
  \item Accept if and only if $B \geq B_0$.
  \end{enumerate}

  First we show that $M$ can be implemented on a polynomial number of processors with parallel running time $\log^k n$ and with at most $O(\log^{j + r} n)$ nondeterministic gates.
  $M$ uses exactly $2^{r \loglog n + t}\cdot q \log^j n$ nondeterministic gates (all in the first step), or more simply, $\log^r n \cdot 2^t q \log^j n$, which is in $O(\log^{j + r} n)$.
  In step 2, $M$ runs $2^{r \loglog n + t}$, or more simply $2^t \log^r n$ (which is bounded above by a polynomial in $n$), instances of $V$ in parallel; the parallel running time of $V$ itself is $O(\log^k)$, so the parallel running time of this step in $M$ is the same.
  In step 3, $M$ computes the sum of $2^{r \loglog n + t}$ numbers, or more simply $2^t \log^r n$ numbers (each a single bit, so the total size of $B$ is $\log 2^t \log^r n$, which equals $r \loglog n + t$), and computes the value of $\frac{1}{2} 2^{r \loglog n + t}$, or $\frac{1}{2} 2^t \log^r n$.
  The sum of $2^t \log^r n$ numbers can be computed by an \NC{} circuit of size $O(\log^r n)$ and depth $O(r \loglog n)$ \cite[Theorem~2.6.1]{savage98}.

  Now we show the correctness of $M$.
  If $x \in L$, then all instances of $V$ accept, so $B = 2^t \log^r n \geq \frac{1}{2} 2^t \log^r n = B_0$, and hence $M$ accepts.
  If $x \notin L$, then fewer than half of the instances of $V$ accept, so $B < \frac{1}{2} 2^t \log^r n = B_0$, and hence $M$ rejects.
  Therefore we have shown that $M$ is a correct algorithm for $L$ which uses a polynomial number of processors, has $O(\log^k n)$ running time, and uses $O(\log^{j + r} n)$ nondeterministic gates.
...
Since we will be interested only in the subclasses of $\NNC(\polylog)$, there must also be a polylogarithmic upper bound on the number of nondeterministic gates required, and hence the length of $\rho$ must be in $O(\loglog n)$.

\begin{corollary}\label{cor:pcpinnnck}
  For all $k \in \mathbb{N}$ with $k \geq 1$,
  \begin{enumerate}
  \item $\PCP^{\NC^k}[0, O(\log n)] = \NNC^k(\log n) = \NC^k$,
  \item $\PCP^{\NC^k}[\loglog n + t, O(1)] = \NNC^k(\log n) = \NC^k$ for all non-negative integers $t$, and
  \item $\PCP^{\NC^k}[O(\loglog n), O(\polylog)] \subseteq \NNC^k(\polylog)$.
  \end{enumerate}
\end{corollary}
...
\begin{corollary}\label{cor:pcpinnnc}
  \mbox{}
  \begin{enumerate}
  \item $\PCP^{\NC}[0, O(\log n)] = \NNC(\log n) = \NC$,
  \item $\PCP^{\NC}[\loglog n + t, O(1)] = \NNC(\log n) = \NC$ for all non-negative integers $t$, and
  \item $\PCP^{\NC}[O(\loglog n), O(\polylog)] \subseteq \NNC(\polylog)$.
  \end{enumerate}
\end{corollary}
...
\begin{corollary}
  ...
  \begin{equation*}
    \PCP^{\NC^k}[r \loglog n, q \log^j n] \subseteq \DSPACE(O(\log^{\max(k, r + j)} n)).
  \end{equation*}

  Furthermore,
  \begin{equation*}
    \PCP^{\NC}[O(\loglog n), O(\polylog)] \subseteq \polyL.
  \end{equation*}
\end{corollary}
...
\begin{theorem}\label{thm:equiv}
  ...
  \begin{enumerate}
  \item $\NNC^k(\log^d n) = \NC^k$.
  \item $\NNC^k(\log^d n) = \PCP^{\NC^k}[d \loglog n + t, q]$ for some non-negative integer $t$.
  \item There is a positive integer $c$ with $c > 1$ such that \textsc{$\frac{1}{2}$-gap $c$-CSP} is hard for $\NNC^k(\log^d n)$ under $\leq_m^{NC^k}$ reductions that produce instances with $O(\log^d n)$ variables and $O(\log^d n)$ constraints.
  \end{enumerate}
\end{theorem}
...
\begin{corollary}
  ...
  \begin{enumerate}
  \item $\NNC(\polylog) = \NC$.
  \item $\NNC(\polylog) = \PCP^{\NC}[O(\loglog n), O(1)]$.
...
If the $\NNC(\polylog)$ hierarchy exhibited an upward collapse property, then we could immediately improve \autoref{thm:equiv} to state that $\NNC^k(\polylog) = \NC^k$ if and only if $\NNC^k(\log^d n) = \PCP^{\NC^k}[d \loglog n + t, q]$.
...
\begin{corollary}
  \mbox{}
  \begin{enumerate}
  \item If $\NNC^k(\log^d) = \PCP^{\NC^k}[d \loglog n + t, q]$ for some non-negative integer $t$ and some positive integer $q$, then $\NP \subseteq \DTIME(2^{n^{\frac{1}{d}}})$.
  \item If $\NNC(\polylog) = \PCP^{\NC}[O(\loglog n), O(1)]$, then $\NP \subseteq \SUBEXP$.
  \end{enumerate}
\end{corollary}
\begin{proof}
...
With this derandomization, we will show a simulation of $O(\loglog n)$ bits of randomness by $\loglog n + O(1)$ bits of randomness.
...
\begin{lemma}\label{lem:badsubset}
  Let $L \in \PCPcs{\NC^k}{1}{\frac{1}{16}}{\alpha \loglog n}{O(1)}$ for some integer $\alpha$ greater than 1, and let $V$ be the corresponding PCP verifier.
  Let $\ell = 2^{\loglog n}$.
...
\begin{lemma}\label{lem:16}
  ...
  \begin{equation*}
    \PCPcs{\NC^k}{1}{\frac{1}{2}}{O(\loglog n)}{O(1)}
    \subseteq
    \PCPcs{\NC^k}{1}{\frac{1}{16}}{O(\loglog n)}{O(1)}.
  \end{equation*}
\end{lemma}
...
\begin{lemma}[{\cite[Lemma~4]{fs96}}]\label{lem:derandomize}
  ...
  \begin{equation*}
    \PCP^{\NC^k}[O(\loglog n), O(1)]
    \subseteq
    \PCP^{\NC^k}[\loglog n + O(1), O(1)].
  \end{equation*}
\end{lemma}
\begin{proof}
  Let $L \in \PCPcs{\NC^k}{1}{\frac{1}{2}}{O(\loglog n)}{O(1)}$.
  By \autoref{lem:16}, we know $L$ is also in $\PCPcs{\NC^k}{1}{\frac{1}{16}}{O(\loglog n)}{O(1)}$.
  ...
  Let $\ell = 2^{\loglog n}$ and let $G_\ell$ be a $d$-regular very explicit spectral expander on $\ell$ vertices, for some positive integer $d$.
  ...
  Since $G_\ell$ has exactly $2^{\loglog n}$, or more simply $\log n$, vertices, the total computation time for the random walk is $O(\alpha c \log n)$, which is in $O(\log n)$.
  Since $V$ is an $\NC^k$ algorithm, the simulation of $V$ can be performed by a polynomial size, $O(\log^k n)$ depth circuit.
  Therefore the overall time required by $V'$ is $O(\log n) + O(\log^k n)$, which is in $O(\log^k n)$ for all $k \geq 1$.
  ...
  To summarize, we have shown that $V'$ uses exactly $\loglog n + \alpha c \log d$ bits of randomness, makes $O(1)$ queries to the proof string, can be implemented by an $\NC^k$ circuit, and has completeness 1 and soundness $\frac{1}{2}$.
  Therefore, $L \in \PCP^{\NC^k}[\loglog n + O(1), O(1)]$.
\end{proof}

Observe that we can allow less explicit expanders in the above proof, as long as the time required to compute the neighbor of each node is in $O(|V|^k)$, where $k$ is the depth of the $\NC$ circuit, but linear time certainly suffices.
...
\begin{lemma}\label{lem:1}
  For all positive integers $k$ and all non-negative rationals $d$, we have $\NNC^k(\log^d n) = \NC^k$ if and only if there is a non-negative integer $t$ such that $\NNC^k(\log^d n) = \PCP^{\NC^k}[d \loglog n + t, q]$.
\end{lemma}
\begin{proof}
  First, suppose $\NNC^k(\log^d n) = \NC^k$.
  Then we have
  \begin{equation*}
    \NNC^k(\log^d n)
    = \NC^k
    = \PCP^{\NC^k}[0, 0]
    \subseteq \PCP^{\NC^k}[d \loglog n, q].
  \end{equation*}
  Combine this with \autoref{prop:pcpinnnc} to achieve equality.
  In this case, we choose $t = 0$.

  For the converse, suppose $\NNC^k(\log^d n) = \PCP^{\NC^k}[d \loglog n + t, q]$.
  Then we have
  \begin{align*}
    \NNC^k(\log^d n) & = \PCP^{\NC^k}[d \loglog n + t, q] \\
    & \subseteq \PCP^{\NC^k}[d' \loglog n, q] & \text{(for some } d' \geq d \text{)} \\
    & \subseteq \PCP^{\NC^k}[\loglog n + t', q] & \text{(by \autoref{lem:derandomize})} \\
    & = \NC^k & \text{(by \autoref{cor:pcpinnnck})}.
  \end{align*}
  Thus $\NNC^k(\log^d n) = \NC^k$.
\end{proof}

\begin{lemma}\label{lem:constraintsnc}
  Suppose $d$ and $t$ are non-negative integers.
  Let $L$ be a language in $\PCP^{\NC^k}[d \loglog n + t, O(1)]$.
  Then there is a $\leq_m^{NC^k}$ reduction from $L$ to \textsc{$\frac{1}{2}$-gap $q$-CSP}, for some integer $q$ greater than 1.
  Furthermore, the reduction yields $O(\log^d n)$ variables and $O(\log^d n)$ constraints on inputs of length $n$.
\end{lemma}
\begin{proof}
  ...
  Since the verifier $V$ now uses only $d \loglog n + t$ bits of randomness, the reduction enumerates (in parallel) each of the $2^{d \loglog n + t}$ possible random strings of length $d \loglog n + t$, instead of the $2^{r \log n}$ possible strings of length $r \log n$.
  Observe that the number of variables must be $q 2^{d \loglog n + t}$, which equals $q 2^t \log^d n$, and the number of constraints is $2^t \log^d n$, both members of $O(\log^d n)$.
\end{proof}

\begin{lemma}\label{lem:2}
  For all positive integers $k$ and all non-negative rationals $d$, $\NNC^k(\log^d n) = \PCP^{\NC^k}[d \loglog n + t, q]$ for some non-negative integer $t$ if and only if there exists an integer $c$ with $c > 1$ such that \textsc{$\frac{1}{2}$-gap $c$-CSP} is hard for $\NNC^k(\log^d n)$ under $\leq_m^{NC^k}$ reductions that produce instances with $O(\log^d n)$ variables and $O(\log^d n)$ constraints.
\end{lemma}
\begin{proof}
  ...
  Observe that the PCP verifier in the proof that the latter statement implies the former requires $d \loglog n + \log c$ bits of randomness to select one of the $c \log^d n$ constraints uniformly at random, where $c$ is a constant, so we choose $t = \log c$.
...
\section{Probabilistically checkable proof systems for polynomial time machines with limited nondeterminism}

The results in the previous sections can be adapted in a straightforward manner to the $\NP(\polylog)$ hierarchy, the class of languages decidable by polynomial time Turing machines augmented with a polylogarithmic amount of nondeterminism.
More formally, $\NP(\log^k n)$ is the class of languages for which there is a Turing machine that decides it in polynomial time using $O(\log^k n)$ bits of nondeterminism on inputs of length $n$.
The class $\NP(\polylog)$ is $\cup_{k \in \mathbb{N}}{\NP(\log^k n)}$.
(These classes have many other names, including $\betakP$ \cite{dt90}, $\P_{\log^k n}$ \cite{kf80}, $\GC(\log^k n, \P)$ \cite{cc97}, etc.)
Just as
\begin{equation*}
  \NC = \NNC(\log n) \subseteq \NNC(\polylog) \subseteq \NNC(\poly) = \NP,
\end{equation*}
we have
\begin{equation*}
  \P = \NP(\log n) \subseteq \NP(\polylog) \subseteq \NP(\poly) = \NP.
\end{equation*}

\begin{proposition}
  For all non-negative integers $j$ and $t$, and all positive rationals $q$ and $r$,
  \begin{equation*}
    \PCP^{\P}[r \loglog n + t, q \log^j n] \subseteq \NP(\log^{r + j} n)
  \end{equation*}
\end{proposition}
...
\begin{theorem}
  \mbox{}
  \begin{enumerate}
  \item For all positive integers $k$ and $q$, we have $\NP(\log^k n) = \PCP^{\P}[k \loglog n + t, q]$ for some non-negative integer $t$ if and only if $\NP(\log^k n) = \P$.
  \item $\NP(\polylog) = \PCP^{\P}[O(\loglog n), O(1)]$ if and only if $\NP(\polylog) = \P$.
  \end{enumerate}
\end{theorem}
...
\begin{corollary}
  \mbox{}
  \begin{enumerate}
  \item For all positive integers $k$ and $q$, if $\NP(\log^k n) = \PCP^{\P}[k \loglog n + t, q]$ for some non-negative integer $t$ then $\NP \subseteq \DTIME(2^{n^{\frac{1}{k}}})$.
  \item If $\NP(\polylog) = \PCP^{\P}[O(\loglog n), O(1)]$ then $\NP \subseteq \SUBEXP$.
  \end{enumerate}
\end{corollary}

We did not focus on this very important family of classes in this work because our motivation was progress toward probabilistically checkable proof systems for \P, and each class in the $\NP(\polylog)$ hierarchy trivially includes \P.
