...
$\NNC^k(\lg^d n)$ has complete problems under logarithmic space many-one \cite[Theorem~3.6]{cc97}, so that part of the hypothesis in item 2 of the next proposition makes sense.
...
%% A problem hard for $\NNC(\polylog)$ under polylogarithmic size bounded reductions could also be considered unlikely because the reduction is length-decreasing (for sufficiently large instances).
