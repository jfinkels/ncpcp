...
Recall now \autoref{eq:pcpchain},
\begin{equation*}
  \PCP^\NC[O(\loglog n), O(1)] \subseteq \NNC(\polylog) \subseteq \PCP^\NC[O(\log n), O(1)].
\end{equation*}
This leaves the possibility that $\NNC(\polylog) = \PCP^\NC[\poly(\loglog n), O(1)]$, or $\NNC(\polylog) = \PCP^\NC[O(\frac{\log n}{\loglog n}), O(1)]$, for example.
There is also the possibility that $\NNC(\polylog) = \PCP^\NC[O(\loglog n), \polylog]$; a straightforward simulation proves one inclusion of the equality.
\begin{theorem}
  $\PCP^\NC[O(\loglog n), \polylog] \subseteq \NNC(\polylog)$.
\end{theorem}
\begin{proof}
  Let $L$ be a language in $\PCP^\NC[O(\loglog n), \polylog]$, and suppose $V$ is the corresponding PCP verifier that decides it using $r \loglog n$ bits of randomness and makes $q \log^k n$ queries to the proof string.
  Construct $\NNC$ machine $M$ as follows on input $x$.
  \begin{enumerate}
  \item Guess a proof string $\pi$ of length $2^{r \loglog n} q \log^k n$.
  ...
  \end{enumerate}
In the initial step, guessing a proof string requires a polylogarithmic number of bits.
...
Therefore $M$ is an $\NNC(\polylog)$ machine.
...
Next, consider (the high-level description of) the polynomial time many-one reduction from \textsc{$q$-CSP} to \textsc{$\frac{1}{2}$-gap $q$-CSP}: given as input constraint graph $G_0$, compute and output $G_{O(\log n)}$, where $G_{i + 1} = \mathcal{P}(\left(X(G_i)\right)^t)$ and $t \in O(1)$.
